...
### 5.1 技术选择（Phase 5 已定案）

| 事项 | 当前基线 | 定案结论 |
| --- | --- | --- |
| 数据库 | PostgreSQL + asyncpg + Alembic | 开发环境默认切换至 **SQLite（aiosqlite）**，PostgreSQL 保留为可选。MySQL（asyncmy）本阶段仅验证兼容性，留待 Phase 10。`utc_now()` 改为返回 naive datetime，`ModelConfig` 的 PostgreSQL 局部唯一索引移除。 |
| 实时通信 | 尚未建设 | **WebSocket** 选定为聊天实时通信协议。SSE 保留给 AI 流式推送。端点 `ws://host/api/v1/ws`，鉴权复用 HTTP Cookie，协议定义见 `docs/api/ws-chat.md`。 |
| 文件存储 | 尚未建设 | 本地文件系统演示存储（`storage/YYYY/MM/DD/`），SHA-256 去重、引用计数管理。大小限制 10 MB，类型白名单（jpeg/png/gif/webp/pdf/txt/zip）。通过 API 鉴权服务，不直接暴露静态文件。 |
| 用户注册 | MVP 不开放注册 | 接口 `POST /api/v1/auth/register`（Phase 6 实现）。用户名 4-30 字符字母开头，密码 12-128 字符，注册后分配 `default_user` 角色。同 IP 每小时限 3 次。 |
| 自动化 | 仅支持手动采集 | **APScheduler**（AsyncIOScheduler + SQLAlchemyJobStore）选定为定时调度器。cron 表达式、作业持久化、跨数据库兼容。完整管理端在 Phase 9 实现。 |
| 视觉手势 | 尚未建设 | 暂缓定案，Phase 9 再做浏览器能力调研。 |

### 5.2 并行工作包（已完成）

| 工作流 | 工作包 | 验收状态 |
| --- | --- | --- |
| A | 数据库兼容性验证；注册安全设计；聊天权限代码；实时通信鉴权；文件安全规则 | ✅ 全量 120 项测试在 SQLite 下通过；Alembic 迁移链 upgrade/downgrade 通过；WebSocket 回声端点可用 |
| B | 聊天、数字员工、工具、舆情和自动化领域模型草案；冻结实体边界 | ✅ 16 张表实体定义已冻结，写入 `docs/context/decisions.md`；16 项扩展权限代码已写入 `bootstrap.py` |
| C | 用户侧主入口信息架构；智能问数/智能聊天入口；聊天和大屏交互原型 | ✅ `UserLayout.vue` 骨架、`/qa` 移至 UserLayout、`/chat` 占位路由、Phase 6-9 管理端导航注释已完成 |

**完成门槛**（Status: ✅ 已完成）：

- ✅ 新增扩展模块正式契约，不直接复用早期原型路由或表结构。
- ✅ SQLite/MySQL 兼容策略经过最小代码验证（`uv run pytest tests/ -q` 120 passed on SQLite）。
- ✅ 后续 Phase 6-9 所需权限代码、系统导航和迁移顺序已冻结。
