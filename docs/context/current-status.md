...
正式产品处于 **Phase 6（用户入口与即时通信核心）** 阶段。Phase 5（课程扩展底座）已合并到 main，其全部前置工作（数据库兼容、权限代码、WebSocket 基座、注册契约、文件骨架、调度器骨架和领域模型冻结）已完成。
...
Phase 6 完成后，Phase 7（聊天增强、数字员工与后台治理）可以开始实施。待完成的功能范围见 `docs/planning/course-assignment-delivery-plan.md`。
