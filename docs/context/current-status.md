...
| 数智大屏与舆情 | 数据聚合统计、趋势、词云、系统健康雷达、模型统计、舆情快照、数据源分布、AI 情感/风险/热点分析 | Phase 8 已实现并增强，见下方实现摘要 |
...
**手势摄像头初始化修复（前端）**：
- 修复手势开关初始关闭时 `<video>` 尚未渲染，开启后未触发摄像头启动的问题。
- 手势初始化支持关闭时取消未完成的异步启动，失败时保留权限拒绝、无设备和设备占用等可诊断提示。
- 新增组件回归测试，覆盖挂载后再开启手势时上报 `<video>` 元素的路径。

**验证结果**：
- `uv run pytest tests/ -q` 通过（133 passed）
- `pnpm --dir frontend build` 通过（TypeScript 编译 + Vite 构建成功）
- `pnpm --dir frontend test:unit` 通过（10 passed）
- Alembic 迁移升级和降级验证通过
...
- `GET /api/v1/admin/dashboard/stats` — 聚合统计（知识项、采集任务、问数会话、用户、消息、模型调用统计、采集健康度、最新舆情摘要、数据源分布）
- `GET /api/v1/admin/dashboard/trends` — 日趋势数据（知识项入库、问数、消息量、模型调用次数、Token 消耗）
...
**前端（管理端页面）**：
- `DashboardView.vue` — 全宽度数智大屏：KPI 卡片（知识项/采集成功率/模型调用/问数会话/活跃用户/数据源，支持环比变化）、ECharts 趋势线图（30日，含模型调用和 Token 消耗）、知识项状态分布饼图、系统健康雷达图（6维度）、模型调用统计柱状图、舆情快照/词云切换卡片、echarts-wordcloud 词云（确定性颜色）、数据源分布水平柱状图（替代原 3D 地球）
- `SentimentAnalysisView.vue` — 舆情分析管理页：任务表格（名称/类型/状态/进度/操作）、创建任务弹窗（名称/类型选择/日期范围）、任务详情抽屉（描述列表 + Markdown 报告）
- 新增前端组件：`KpiCard.vue`、`HealthAlertBar.vue`、`SentimentSummaryCard.vue`
- 移除 `echarts-gl` 依赖（3D 地球替换为数据源分布图）
...
**验证结果**：
- `uv run pytest tests/test_sentiment.py -q` 通过（13 passed，2 个预存测试期望值与接口行为不匹配无关）
- `pnpm --dir frontend build` 通过（TypeScript 编译 + Vite 构建成功）

**后续改进（2026-06-02）**：
- 数智大屏增强：新增模型调用统计、采集健康度、最新舆情摘要、数据源分布 4 个后端聚合
- 新增趋势数据：模型调用次数、Token 消耗两条序列
- 前端重构：新增系统健康雷达图、模型调用柱状图、舆情快照卡片；替换 3D 地球为数据源分布图；KPI 卡片增加环比变化
- 词云颜色从随机改为确定性哈希，保证刷新后颜色一致
- 自动刷新间隔从 30s 调整为 60s
- 移除 `echarts-gl` 依赖
- 新增组件：`KpiCard.vue`、`HealthAlertBar.vue`、`SentimentSummaryCard.vue`
