# 待解决问题与风险

## 实现前必须确认

| 事项 | 当前判断 | 处理时机 |
| --- | --- | --- |
| 内容检索方式 | 首版已采用 `available` 内容的关键词检索作为 MVP 方案；复杂全文/向量策略仍不进入首版范围 | MVP 后续优化时再评估 |
| Git 仓库与主分支保护 | Git 仓库和 `main` 已存在；远端主分支保护与评审门禁是否落地待确认 | 首批功能合并前验证分支保护与评审流程 |
| 登录接口防暴力保护 | 已统一认证失败响应，但尚未实现失败频率限制、失败审计或等效的口令猜测防护 | 生产部署与安全验收前确定策略并实现 |
| SQLite/MySQL 与现有 PostgreSQL 基线的兼容方式 | 课程任务书要求默认 SQLite、后台切换 MySQL；当前实现使用 PostgreSQL + asyncpg + Alembic，不能只增加配置项完成切换 | Phase 5 先验证驱动、迁移、连接管理和关键约束兼容性，再批量新增聊天表 |
| 即时通信实时协议 | 任务书要求私聊、群聊、未读和数字员工群内回复；当前尚无实时通信层 | Phase 5 在 WebSocket 等方案中定案鉴权、断线重连、消息补拉和失败策略 |
| 聊天文件存储 | 任务书要求文件收发、后台集中管理和避免重复存储 | Phase 5 定案本地演示存储、SHA-256 去重、权限校验、大小限制和下载方式 |

## 已解决的实施选择

| 事项 | 结论 | 记录位置 |
| --- | --- | --- |
| Web 框架、ORM 与迁移 | FastAPI + SQLAlchemy AsyncSession + Alembic | `docs/context/decisions.md` |
| 首版关系型数据库 | PostgreSQL，开发环境由 Docker Compose 提供 | `docs/context/decisions.md` |
| 普通用户账户开通方式 | 由管理员创建，MVP 不开放自助注册 | `docs/context/decisions.md` |
| 前端实现方式 | Vue 3 + TypeScript + Vite + Pinia + Vue Router + Element Plus，`pnpm` 管理 | `docs/context/decisions.md` |
| Phase 2 智能瞭望与数据仓库联调 | 数据源、采集任务和数据仓库治理页面已对接 Phase 2 B 后端接口，并通过后端 pytest、前端构建和单元测试 | `docs/context/current-status.md` |

## Phase 4 C 当前风险（2026-05-30）

| 风险 | 当前判断 | 后续处理 |
| --- | --- | --- |
| MVP 端到端演示依赖本地环境 | 后端集成测试需要 PostgreSQL；前端构建和单元测试需要本机可用 `pnpm` 与 `node_modules` | Phase 4 C 验收时需启动 `docker compose up -d postgres`，并在前端环境安装依赖后执行构建与单元测试 |
| 流式问数写操作 CSRF 收口 | 已发现并修复 `questions/stream` 缺少后端 CSRF 依赖的问题 | 保留集成测试覆盖，Phase 4 继续按 MVP 安全门槛复核所有写操作 |

## 高风险要求

| 风险 | 必须采取的控制 |
| --- | --- |
| 模型或来源凭据泄露 | 加密存储、脱敏响应、不进入日志与审计详情 |
| 采集造成 SSRF | 域名白名单、地址解析校验、重定向复查、超时与大小限制 |
| 权限仅在页面控制 | 所有管理 API 执行服务端权限判定 |
| AI 使用不可信采集内容 | 将内容作为资料处理，防止内容注入改变系统指令 |
| 旧原型秘密或演示配置被复用 | 重写时不复制任何原型凭据；若曾在外部使用过演示秘密，应予以轮换 |

## 待记录的实施问题

后续开发遇到阻塞、兼容限制、安全取舍或未完成验收项时追加在此，并链接对应决策或任务。

## Phase 1 B 发现的问题（2026-05-27）

| 优先级 | 问题 | 建议方案 |
| --- | --- | --- |
| 中 | `set_default_model` 并发竞态：多个并发请求可能同时设置不同的默认模型 | 在数据库层添加部分唯一约束，或在应用层使用行锁定（`SELECT ... FOR UPDATE`） |
| 低 | 测试 API key 格式：当前使用 `sk-` 前缀可能触发密钥扫描 | 建议改为 `test-` 前缀以避免误报 |

