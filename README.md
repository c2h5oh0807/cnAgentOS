...
- 数据库：SQLite（开发默认），迁移由 Alembic 管理；PostgreSQL 通过 `DATABASE_URL` 环境变量可选切换。
...
# 执行数据库迁移（默认 SQLite，自动创建 backend/data/cnagentos.db）
...
# 运行 API 服务
uv run uvicorn cnagentos.app:create_app --factory --reload
